...
public class GrabDropScript : MonoBehaviour, InteractionListenerInterface
{
    ...
    Vector3[] objPosition=new Vector3[3];
	...
	void Start()
	{
        ...
        {
            objPosition[i] = draggableObjects[i].transform.position;
        }
					...
					if(Physics.Raycast(ray, out hit))
					{
						foreach(GameObject obj in draggableObjects)
						{
							if(hit.collider.gameObject == obj)
							{
                                ...
                                //draggedObject.GetComponent<ObjectShake>().enabled = false;
			...
			else
			{
				...
				if(isReleased)
				{
					...
					if(useGravity)
					{
                        ...
                        GameObject resetObject = draggedObject;
                        objCount++;
                        if (objCount==3)
                        {
                            objCount = 0;
                            Debug.Log("3333333333");
                            StartCoroutine(WaitTime(resetObject));
    ...
    IEnumerator WaitTime(GameObject resetObject)
    {
